...
class _SplitTunnelDialogState extends State<_SplitTunnelDialog> {
  ...
  Future<List<_IndexedSplitTunnelApp>> _loadIndexedAppCatalog({
    ...
    return apps.map(_IndexedSplitTunnelApp.fromApp).toList(growable: false);
  }
...
class _DomainSplitTunnelDialogState extends State<_DomainSplitTunnelDialog> {
  ...
  late final TextEditingController _udpPortController;
  late final FocusNode _domainFocusNode;
  late final FocusNode _udpPortFocusNode;

  @override
  void initState() {
    super.initState();
    _domainController = TextEditingController();
    _udpPortController = TextEditingController();
    _domainFocusNode = FocusNode();
    _udpPortFocusNode = FocusNode();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final udpPorts = controller.domainSplitTunnelUdpPorts;
            ...
            if (domainSplitTunnelEnabled) ...<Widget>[
              ...
              Row(
                children: <Widget>[
                  Expanded(
                    child: ListenableBuilder(
                      listenable: _udpPortFocusNode,
                      builder: (context, _) => TextField(
                        controller: _udpPortController,
                        focusNode: _udpPortFocusNode,
                        enabled: canEditDomains,
                        keyboardType: TextInputType.number,
                        decoration: InputDecoration(
                          hintText: _udpPortFocusNode.hasFocus
                              ? null
                              : strings.domainSplitTunnelUdpPortInputHint,
                        ),
                        onSubmitted: canEditDomains
                            ? (_) => _addUdpPort()
                            : null,
                  ...
                  const SizedBox(width: 8),
                  ValueListenableBuilder<TextEditingValue>(
                    valueListenable: _udpPortController,
                    builder: (context, value, _) {
                      final canAddPort =
                          canEditDomains && value.text.trim().isNotEmpty;
                      return IconButton.filledTonal(
                        tooltip: strings.domainSplitTunnelUdpPortAddTooltip,
                        onPressed: canAddPort ? _addUdpPort : null,
                        icon: const Icon(Icons.add_rounded),
                      );
                    },
                  ),
              ...
              const SizedBox(height: 14),
              _SplitTunnelSectionHeader(
                label: strings.domainSplitTunnelUdpPortsLabel,
                countLabel: strings.domainSplitTunnelUdpPortsCount(
                  udpPorts.length,
                ),
              ),
              const SizedBox(height: 8),
              if (udpPorts.isEmpty)
                Padding(
                  ...
                  child: _SplitTunnelEmptyState(
                    message: strings.domainSplitTunnelNoUdpPorts,
                  ),
                )
              else
                Wrap(
                  ...
                  children: <Widget>[
                    for (final port in udpPorts)
                      InputChip(
                        avatar: const Icon(Icons.settings_ethernet_rounded),
                        label: Text('UDP ${port.value}'),
                        onDeleted: controller.canChangeSplitTunnel
                            ? () {
                                setState(() {
                                  controller.removeDomainSplitTunnelUdpPort(
                                    port,
                                  );
                                });
                              }
  ...
  void _addUdpPort() {
    final input = _udpPortController.text;
    setState(() {
      widget.controller.addDomainSplitTunnelUdpPortInput(input);
      _udpPortController.clear();
    });
  }
}
