...
class _MobileSubscriptionProfileCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final tcpPingInfo = source.tcpPingLatencyInfoForProfile(profileIndex);
    final tcpPingLatency = source.tcpPingLatencyForProfile(profileIndex);
    final showPingResult =
        tcpPingLatency != null || tcpPingInfo?.isFailure == true;
            ...
            trailing: Row(
              ...
              children: <Widget>[
                if (showPingResult) ...<Widget>[
                  _TcpPingStatusLabel(
                    info: tcpPingInfo,
                    fallbackMilliseconds: tcpPingLatency,
                  ),
...
class _TcpPingStatusLabel extends StatelessWidget {
  const _TcpPingStatusLabel({
    required this.info,
    required this.fallbackMilliseconds,
  });

  final TcpPingProfileLatency? info;
  final int? fallbackMilliseconds;

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final latencyMs = info?.latencyMs ?? fallbackMilliseconds;
    final isFailure = info?.isFailure == true;
    final isStale = info?.hasLatency == true && info!.isStale();
    final label = isFailure
        ? 'failed'
        : isStale
        ? 'stale'
        : '${latencyMs ?? 0} ms';
    final latencyColor = isFailure
        ? theme.colorScheme.error
        : isStale
        ? theme.colorScheme.onSurfaceVariant
        : _tcpPingLatencyColor(latencyMs ?? 0);
...
class _SourceMenuController {
    ...
    if (source.isSubscription) {
      items.add(
        PopupMenuItem<SourceMenuAction>(
          value: SourceMenuAction.fastest,
          enabled: controller.canPingSource(source.id),
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: <Widget>[
              Icon(Icons.speed_rounded, color: scheme.onSurfaceVariant),
              const SizedBox(width: 10),
              Text(strings.autoFastestAction, style: menuLabelStyle),
            ],
          ),
        ),
      );
  ...
  void handleAction(BuildContext context, SourceMenuAction action) {
    switch (action) {
      case SourceMenuAction.fastest:
        controller.setFastestProfileSelection(source.id);
        break;
...
class _QuickSourceTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final tcpPingInfo = source.tcpPingLatencyInfoForProfile(
      source.selectedProfileIndex,
    );
    final showPingResult =
        tcpPingLatency != null || tcpPingInfo?.isFailure == true;
