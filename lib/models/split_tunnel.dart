...
class SplitTunnelDomain {
  ...
  bool get isIp => _normalizeIpOrCidr(matchSuffix) != null;
...
class SplitTunnelUdpPort {
  const SplitTunnelUdpPort({required this.value});

  factory SplitTunnelUdpPort.fromInput(String input) {
    final value = input.trim().replaceAll('\uFEFF', '');
    if (!_kPortNumberPattern.hasMatch(value)) {
      throw const FormatException('UDP port is invalid.');
    }
    final port = int.tryParse(value);
    if (port == null || port < 1 || port > 65535) {
      throw const FormatException('UDP port is out of range.');
    }
    return SplitTunnelUdpPort(value: port);
  }

  final int value;

  String get id => value.toString();

  SplitTunnelUdpPort get normalized => SplitTunnelUdpPort.fromInput(
    value.toString(),
  );
  ...
  factory SplitTunnelUdpPort.fromJson(Map<String, dynamic> json) {
    return SplitTunnelUdpPort.fromInput(
      (json['value'] ?? json['port'] ?? '').toString(),
    );
  }
}
...
class DomainSplitTunnelSettings {
  const DomainSplitTunnelSettings({
    ...
    this.udpPorts = const <SplitTunnelUdpPort>[],
  });

  final SplitTunnelMode mode;
  final List<SplitTunnelDomain> domains;
  final List<SplitTunnelUdpPort> udpPorts;

  bool get isEnabled => mode != SplitTunnelMode.off;
  bool get hasSelectedDomains => domains.isNotEmpty;
  bool get hasUdpPorts => udpPorts.isNotEmpty;
  ...
  DomainSplitTunnelSettings get normalized {
    ...
    final normalizedUdpPorts = <SplitTunnelUdpPort>[];
    final seenPorts = <int>{};
    for (final port in udpPorts) {
      try {
        final normalized = port.normalized;
        if (!seenPorts.add(normalized.value)) {
          continue;
        }
        normalizedUdpPorts.add(normalized);
      } on FormatException {
        continue;
      }
    }
    normalizedUdpPorts.sort((left, right) => left.value.compareTo(right.value));
  ...
  factory DomainSplitTunnelSettings.fromJson(Map<String, dynamic>? json) {
    ...
    return DomainSplitTunnelSettings(
      ...
      domains: ((json['domains'] as List<dynamic>?) ?? const <dynamic>[])
          ...
          .toList(growable: false),
      udpPorts: ((json['udpPorts'] as List<dynamic>?) ?? const <dynamic>[])
          .map((item) {
            try {
              if (item is int) {
                return SplitTunnelUdpPort(value: item);
              }
              if (item is String) {
                return SplitTunnelUdpPort.fromInput(item);
              }
              if (item is Map<String, dynamic>) {
                return SplitTunnelUdpPort.fromJson(item);
              }
              if (item is Map) {
                return SplitTunnelUdpPort.fromJson(
                  item.map((key, value) => MapEntry(key.toString(), value)),
                );
              }
            } on FormatException {
...
_ParsedDomainInput _parseDomainInput(String input) {
  var value = input
      ...
      .toLowerCase();
  if (value.isEmpty) {
    throw const FormatException('Domain or IP is empty.');
  }

  final literalIp = _normalizeIpOrCidr(value);
  if (literalIp != null) {
    return _ParsedDomainInput(displayValue: literalIp, matchSuffix: literalIp);
  }
  ...
  final normalizedIp = _normalizeIpOrCidr(value);
  if (normalizedIp != null) {
    return _ParsedDomainInput(
      displayValue: normalizedIp,
      matchSuffix: normalizedIp,
    );
  }
...
String? _normalizeIpOrCidr(String input) {
  var value = input.trim().replaceAll('\uFEFF', '').toLowerCase();
  if (value.isEmpty) {
    return null;
  }
  ...
  if (value.startsWith('[')) {
    ...
    if (end > 0) {
      value = value.substring(1, end) + value.substring(end + 1);
    }
  }

  value = _stripPort(value);
  final slashIndex = value.indexOf('/');
  final host = slashIndex >= 0 ? value.substring(0, slashIndex) : value;
  ...
  final prefixText = value.substring(slashIndex + 1);
  if (!_kPortNumberPattern.hasMatch(prefixText)) {
    return null;
  }
  final prefix = int.tryParse(prefixText);
  if (prefix == null) {
    return null;
  }
