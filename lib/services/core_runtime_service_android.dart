...
extension CoreRuntimeServiceAndroid on CoreRuntimeService {
  ...
  Future<void> _saveAndroidStartPayload({
    ...
    final payload = _buildAndroidStartPayload(
      ...
      profile: profile,
      language: language,
      serverCountryCode: _cachedAndroidServerCountryCode(profile),
      tunIpMode: tunIpMode,
    ...
    unawaited(_refreshAndroidServerCountryCode(profile));
  }
  ...
  String? _cachedAndroidServerCountryCode(ParsedVpnProfile profile) {
    final server = profile.server.trim();
    if (server.isEmpty) {
      return null;
    }
    try {
      final info = _geoIpService.cachedServer(server);
  ...
  Future<void> _refreshAndroidServerCountryCode(
    ...
    try {
      await _geoIpService.resolveServer(server);
    } catch (_) {}
  }
