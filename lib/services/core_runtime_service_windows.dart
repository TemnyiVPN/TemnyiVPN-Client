...
extension CoreRuntimeServiceWindows on CoreRuntimeService {
  ...
  String _buildWindowsNativeRuntimeOptionsJson({
    ...
    return jsonEncode(<String, Object?>{
      ...
      'domainSplitTunnelDomains': domainSplitTunnel.domains
          ...
          .toList(growable: false),
      'domainSplitTunnelUdpPorts': domainSplitTunnel.udpPorts
          .map((port) => port.value.toString())
          .toList(growable: false),
    });
  }
  ...
  Future<bool> ensureWindowsTunPrivileges({TunIpMode? tunIpMode}) async {
    ...
    if (elevated == false) {
      ...
      _rememberAppLog(
        'Windows TUN mode requires Administrator privileges; relaunching TemnyiVPN elevated...',
      );
    ...
    if (elevated == null) {
      _rememberAppLog(
        'Could not determine whether TemnyiVPN is elevated; continuing and letting the core report any permission error.',
      );
    }
