...
extension CoreRuntimeServiceWindowsService on CoreRuntimeService {
  Future<bool> _ensureWindowsTunServiceReady({
    ...
    if (await _pingWindowsTunService()) {
      ...
      _rememberAppLog(
        'TemnyiVPN Service helper is running; Windows TUN will start without relaunching the UI elevated.',
      );
      return true;
    }

    _rememberAppLog('Starting TemnyiVPN Service helper...');
    await _startWindowsTunService();
    final deadline = DateTime.now().add(const Duration(seconds: 4));
    while (DateTime.now().isBefore(deadline)) {
      if (await _pingWindowsTunService()) {
        _windowsTunServiceReady = true;
        _rememberAppLog('TemnyiVPN Service helper is ready.');
        return true;
      }
    ...
    _rememberAppLog(
      'TemnyiVPN Service helper did not become ready; falling back to elevation.',
    );
    return false;
  }
  ...
  Future<Map<String, String>> _runWindowsServiceHelper(
    ...
    try {
      ...
      if (rawResult is! Map) {
        throw StateError(
          'TemnyiVPN service helper native runner returned unexpected result.',
        );
      }
      final result = rawResult.cast<Object?, Object?>();
      ...
      if (result['ok'] != true) {
        throw StateError(
          result['error']?.toString() ??
              'TemnyiVPN Service request failed in native runner.',
        );
      }
      final fields = result['fields'];
      if (fields is! Map) {
        throw StateError(
          'TemnyiVPN service helper native runner returned no response fields.',
        );
      }
