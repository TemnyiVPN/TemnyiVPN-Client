...
const Duration tcpPingQuickTimeout = Duration(milliseconds: 1000);
const int tcpPingQuickMaxConcurrent = 24;
const Duration tcpPingFallbackTimeout = Duration(milliseconds: 2800);
const int tcpPingFallbackMaxConcurrent = 8;
const int tcpPingFallbackTargetLimit = 12;
...
Future<List<TcpPingMeasurement>> measureTcpPingTargets(
  ...
  if (nativeResults.isEmpty && throwOnAllFailed) {
    throw const SocketException('TCP ping failed for all targets.');
  }
  return nativeResults;
}

Future<List<TcpPingMeasurement>> measureTcpPingTargetsFast(
  List<TcpPingTarget> targets, {
  ...
  final quickMeasurements = await measureTcpPingTargets(
    targets,
    maxConcurrent: tcpPingQuickMaxConcurrent,
    timeout: tcpPingQuickTimeout,
    throwOnAllFailed: false,
  );
  final measurementsByKey = <String, TcpPingMeasurement>{
    for (final measurement in quickMeasurements)
      _measurementKey(measurement.profileIndex, measurement.profileKey):
          measurement,
  };
  if (measurementsByKey.length == targets.length) {
    return _sortedMeasurements(measurementsByKey.values);
  }

  final measuredKeys = measurementsByKey.keys.toSet();
  final missingTargets = targets
      .where(
        (target) => !measuredKeys.contains(
          _targetKey(target.profileIndex, target.profileKey),
        ),
      )
      .toList(growable: false);
  final fallbackTargets = <TcpPingTarget>[];
  if (selectedProfileIndex != null) {
    fallbackTargets.addAll(
      missingTargets.where(
        (target) => target.profileIndex == selectedProfileIndex,
      ),
    );
  }
  if (quickMeasurements.isEmpty) {
    for (final target in missingTargets) {
      if (fallbackTargets.length >= tcpPingFallbackTargetLimit) {
        break;
      }
      if (!fallbackTargets.contains(target)) {
  ...
  final fallbackMeasurements = await measureTcpPingTargets(
    fallbackTargets,
    maxConcurrent: tcpPingFallbackMaxConcurrent,
    timeout: tcpPingFallbackTimeout,
    throwOnAllFailed: false,
  );
  for (final measurement in fallbackMeasurements) {
    measurementsByKey[_measurementKey(
      measurement.profileIndex,
      measurement.profileKey,
    )] = measurement;
  }
  if (measurementsByKey.isEmpty) {
    throw const SocketException('TCP ping failed for all targets.');
  }
  return _sortedMeasurements(measurementsByKey.values);
}

bool isTcpPingFastestComparable(ParsedVpnProfile profile) {
  if (profile.protocol == LinkProtocol.hysteria ||
      profile.protocol == LinkProtocol.hysteria2) {
    return false;
  }
  return profile.transport != TransportMode.quic;
}

List<TcpPingMeasurement> _sortedMeasurements(
  Iterable<TcpPingMeasurement> measurements,
) {
  return measurements.toList(growable: false)
    ..sort((left, right) => left.profileIndex.compareTo(right.profileIndex));
}

String _targetKey(int profileIndex, String profileKey) =>
    '$profileIndex\n$profileKey';

String _measurementKey(int profileIndex, String profileKey) =>
    _targetKey(profileIndex, profileKey);
