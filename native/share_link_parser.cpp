...
struct ConfigOptions {
  ...
  std::vector<std::string> domain_split_tunnel_udp_ports;
...
int addrinfo_count(addrinfo* addresses) {
  int count = 0;
...
long long tcp_ping_address_timeout_ms(long long remaining_ms,
                                      int address_count) {
  if (address_count <= 1) {
    return remaining_ms;
  }
  return (std::min)(remaining_ms, 350LL);
}
...
int tcp_connect_latency_ms(const std::string& host, int port, int timeout_ms) {
  ...
  std::unique_ptr<addrinfo, decltype(&freeaddrinfo)> addresses(raw_addresses,
                                                               freeaddrinfo);
  const int address_count = addrinfo_count(addresses.get());
  ...
  for (addrinfo* address = addresses.get(); address != nullptr;
    ...
    const auto remaining_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(deadline - now)
            .count();
    const auto address_timeout_ms =
        tcp_ping_address_timeout_ms(remaining_ms, address_count);
    ...
    timeval timeout{};
    timeout.tv_sec = static_cast<long>(address_timeout_ms / 1000);
    timeout.tv_usec = static_cast<long>((address_timeout_ms % 1000) * 1000);
    const int selected =
...
int tcp_connect_latency_ms(const std::string& host, int port, int timeout_ms) {
  ...
  for (addrinfo* address = addresses.get(); address != nullptr;
    ...
    int polled;
    do {
      polled = ::poll(&waiter, 1, static_cast<int>(address_timeout_ms));
...
ConfigOptions options_from_json(const std::string& json) {
  ...
  options.domain_split_tunnel_udp_ports = json_string_array_value(json, "domainSplitTunnelUdpPorts");
...
std::vector<int> normalized_udp_ports(const ConfigOptions& options) {
  std::vector<int> ports;
  for (const auto& value : options.domain_split_tunnel_udp_ports) {
    const auto normalized = trim(value);
    if (normalized.empty()) continue;
    const int port = parse_int(normalized);
...
void write_port_value(std::ostringstream& json, const std::vector<int>& ports) {
  if (ports.size() == 1) {
    json << ports.front();
    return;
  }
...
bool is_ip_literal_or_cidr(const std::string& value) {
  const auto slash = value.find('/');
  const std::string host = slash == std::string::npos ? value : value.substr(0, slash);
  if (trim(host).empty()) return false;

  unsigned char buffer[16] = {};
  if (inet_pton(AF_INET, host.c_str(), buffer) == 1) {
    if (slash == std::string::npos) return true;
    const auto prefix_text = value.substr(slash + 1);
    if (prefix_text.empty() || prefix_text.find_first_not_of("0123456789") != std::string::npos) return false;
    const int prefix = std::stoi(prefix_text);
    return prefix >= 0 && prefix <= 32;
  }
  if (inet_pton(AF_INET6, host.c_str(), buffer) == 1) {
    if (slash == std::string::npos) return true;
    const auto prefix_text = value.substr(slash + 1);
    if (prefix_text.empty() || prefix_text.find_first_not_of("0123456789") != std::string::npos) return false;
    const int prefix = std::stoi(prefix_text);
...
std::vector<std::string> domain_split_domain_values(const ConfigOptions& options) {
  std::vector<std::string> domains;
  for (const auto& value : options.domain_split_tunnel_domains) {
    const auto normalized = lower(trim(value));
    if (!normalized.empty() && !is_ip_literal_or_cidr(normalized)) domains.push_back(normalized);
  }
  return sorted_unique(domains);
}

std::vector<std::string> domain_split_ip_values(const ConfigOptions& options) {
  std::vector<std::string> ips;
  for (const auto& value : options.domain_split_tunnel_domains) {
    const auto normalized = lower(trim(value));
    if (!normalized.empty() && is_ip_literal_or_cidr(normalized)) ips.push_back(normalized);
  }
  return sorted_unique(ips);
}

std::string domain_matcher_json(const ConfigOptions& options) {
  auto domains = domain_split_domain_values(options);
  auto ips = domain_split_ip_values(options);
  std::vector<std::string> matcher_objects;
  if (!domains.empty()) {
    std::ostringstream object;
    object << "{\"domain_suffix\":";
    write_string_array(object, domains);
    object << '}';
    matcher_objects.push_back(object.str());
  }
  if (!ips.empty()) {
...
std::string udp_port_exclusion_rule_json(const ConfigOptions& options) {
  const auto ports = normalized_udp_ports(options);
  if (ports.empty() || options.domain_split_tunnel_mode == "off") return "";
  std::ostringstream json;
  json << "{\"network\":\"udp\",\"port\":";
  write_port_value(json, ports);
...
void append_sing_box_route_rules(std::ostringstream& json, const ConfigOptions& options) {
  ...
  const auto udp_exclusion_rule = udp_port_exclusion_rule_json(options);
  ...
  if (has_whitelist) {
    json << ",{\"ip_is_private\":true,\"action\":\"route\",\"outbound\":\"direct\"}";
    if (!udp_exclusion_rule.empty()) json << ',' << udp_exclusion_rule;
    json << ",{\"network\":\"udp\",\"port\":443,\"action\":\"reject\",\"method\":\"default\"}";
...
void append_xray_udp_port_exclusion_rule(std::ostringstream& json, const ConfigOptions& options) {
  const auto ports = normalized_udp_ports(options);
  json << "{\"type\":\"field\",\"network\":\"udp\",\"port\":\"";
  for (size_t i = 0; i < ports.size(); ++i) {
    if (i > 0) json << ',';
    json << ports[i];
  }
...
std::string build_xray_config(const Profile& profile, const ConfigOptions& options) {
  ...
  const bool domain_whitelist = options.domain_split_tunnel_mode == "whitelist" && !options.domain_split_tunnel_domains.empty();
  const bool domain_blacklist = options.domain_split_tunnel_mode == "blacklist" && !options.domain_split_tunnel_domains.empty();
  const bool udp_port_exclusions = options.domain_split_tunnel_mode != "off" && !normalized_udp_ports(options).empty();
  ...
  const bool has_rules = use_dns_routing || tun || domain_whitelist || domain_blacklist || udp_port_exclusions;
  if (has_rules) {
    ...
    if (udp_port_exclusions) {
      if (!first) json << ',';
      append_xray_udp_port_exclusion_rule(json, options);
      first = false;
    }
    if (tun) {
      if (!first) json << ',';
