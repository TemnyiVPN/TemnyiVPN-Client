...
void main() {
  test('PersistedAppState round-trips sources and settings', () {
      ...
      sources: <ConfigSource>[
        ConfigSource(
          ...
          tcpPingProfileLatencies: <String, TcpPingProfileLatency>{
            vpnProfileIdentityKey(profile): TcpPingProfileLatency(
              profileKey: vpnProfileIdentityKey(profile),
              latencyMs: 42,
              measuredAt: DateTime.utc(2026, 5, 12, 11),
            ),
          },
          tcpPingLatenciesMs: const <int, int>{0: 42},
          tcpPingLatencyMs: 42,
    ...
    expect(
      restored.sources.first.profileSelectionMode,
      ProfileSelectionMode.fastest,
    );
    ...
    expect(restored.sources.first.tcpPingLatencyForProfile(0), 42);
    expect(
      restored
          .sources
          .first
          .tcpPingLatencyInfoForProfile(0)
          ?.measuredAt,
      DateTime.utc(2026, 5, 12, 11),
    );
