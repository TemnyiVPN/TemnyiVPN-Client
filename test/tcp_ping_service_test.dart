...
void main() {
  ...
  test('quick scan returns reachable targets without failing the whole batch', () async {
    ...
    final measurements = await measureTcpPingTargetsFast(<TcpPingTarget>[
      TcpPingTarget(
        profileIndex: 0,
        profileKey: 'reachable',
        profile: _tcpProfile(server.port),
      ),
      TcpPingTarget(
        profileIndex: 1,
        profileKey: 'closed',
        profile: _tcpProfile(closedPort),
      ),
    ]);
    ...
    expect(measurements.single.latencyMs, greaterThan(0));
  });
