...
void main() {
  ...
  test('fastest mode selects the fresh lowest-latency profile', () async {
    final runtimeService = _FakeCoreRuntimeService();
    final fastProfile = _profileFor('fast.example.com');
    final slowProfile = _profileFor('slow.example.com');
    final controller = VpnController(
      runtimeService: runtimeService,
      appStateStore: _MemoryAppStateStore(
        PersistedAppState(
          ...
          sources: <ConfigSource>[
            ConfigSource(
              ...
              rawInput: 'https://example.com/subscription',
              kind: ConfigSourceKind.subscription,
              profiles: <ParsedVpnProfile>[slowProfile, fastProfile],
              profileSelectionMode: ProfileSelectionMode.fastest,
              tcpPingProfileLatencies: <String, TcpPingProfileLatency>{
                vpnProfileIdentityKey(slowProfile): TcpPingProfileLatency(
                  profileKey: vpnProfileIdentityKey(slowProfile),
                  latencyMs: 120,
                  measuredAt: DateTime.now(),
                ),
                vpnProfileIdentityKey(fastProfile): TcpPingProfileLatency(
                  profileKey: vpnProfileIdentityKey(fastProfile),
                  latencyMs: 32,
                  measuredAt: DateTime.now(),
                ),
              },
    ...
    expect(controller.previewProfile?.server, 'fast.example.com');
    expect(runtimeService.startedProfiles.single.server, 'fast.example.com');
  });

  test('manual mode is not overwritten by cached fastest latency', () async {
    final runtimeService = _FakeCoreRuntimeService();
    final manualProfile = _profileFor('manual.example.com');
    final fastProfile = _profileFor('fast.example.com');
    final controller = VpnController(
      runtimeService: runtimeService,
      appStateStore: _MemoryAppStateStore(
        PersistedAppState(
          ...
          sources: <ConfigSource>[
            ConfigSource(
              ...
              rawInput: 'https://example.com/subscription',
              kind: ConfigSourceKind.subscription,
              profiles: <ParsedVpnProfile>[manualProfile, fastProfile],
              selectedProfileIndex: 0,
              profileSelectionMode: ProfileSelectionMode.manual,
              tcpPingProfileLatencies: <String, TcpPingProfileLatency>{
                vpnProfileIdentityKey(manualProfile): TcpPingProfileLatency(
                  profileKey: vpnProfileIdentityKey(manualProfile),
                  latencyMs: 120,
                  measuredAt: DateTime.now(),
                ),
                vpnProfileIdentityKey(fastProfile): TcpPingProfileLatency(
    ...
    expect(controller.previewProfile?.server, 'manual.example.com');
    expect(runtimeService.startedProfiles.single.server, 'manual.example.com');
  });
...
class _FakeCoreRuntimeService extends CoreRuntimeService {
  final List<ParsedVpnProfile> startedProfiles = <ParsedVpnProfile>[];
