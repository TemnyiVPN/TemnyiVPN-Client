...
bool BuildWindowsServiceRequest(const std::vector<std::string>& args,
  ...
  request->clear();
  if (args.empty()) {
    *error = "Missing TemnyiVPN service command.";
    return false;
  }
  ...
  if (command == "ping") {
  ...
  } else if (command == "manage-interference-processes") {
    AppendServiceField(request, "command", "manage_interference_processes");
    AppendServiceField(
        request, "action",
        WindowsServiceOptionValue(args, "--action", "terminate"));
    AppendServiceArguments(request,
                           WindowsServiceRepeatedOptionValues(args, "--pid"));
    ...
    *error = "Unknown TemnyiVPN service command: " + command;
    return false;
  }
...
ParsedServiceResponse ParseWindowsServiceResponse(
  ...
  if (!decoded_error.empty()) {
    ...
    parsed.error =
        "TemnyiVPN Service request failed with exit " +
