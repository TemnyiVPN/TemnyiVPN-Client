...
// the app asks the service to create "TemnyiVPN TUN" once when it launches.
...
std::string PrewarmTunAdapterNative(
  ...
  if (interface_alias.empty()) {
    interface_alias = L"TemnyiVPN TUN";
  }
  ...
  g_prewarmed_adapter = g_wintun_create_adapter(
      interface_alias.c_str(), L"TemnyiVPN", &kPrewarmedTunGuid);
  if (g_prewarmed_adapter == nullptr) {
