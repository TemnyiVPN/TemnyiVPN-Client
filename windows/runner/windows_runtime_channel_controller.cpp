...
EncodableList PidCsvToEncodableList(const std::string& csv) {
  EncodableList list;
  for (const std::string& item : SplitCommaList(csv)) {
    const int64_t pid = std::strtoll(item.c_str(), nullptr, 10);
    if (pid > 0) {
...
class WindowsRuntimeController {
 public:
  ...
  // Asks the service to create the "TemnyiVPN TUN" wintun adapter now (at
  ...
  EncodableValue PrewarmTunAdapter() {
    if (!EnsureWindowsServiceReady()) {
      return EncodableValue(MakeFailure(
          "service",
          "TemnyiVPN service is not available for TUN pre-warm."));
    }
  ...
  EncodableValue ManageInterferenceProcesses(const EncodableMap& arguments) {
    std::string action;
    if (!ReadString(arguments, "action", &action) ||
        (action != "terminate" && action != "suspend" &&
         action != "resume")) {
      return EncodableValue(MakeFailure(
          "arguments", "Invalid interference process action.",
          ERROR_INVALID_PARAMETER));
    }
    const EncodableValue* pids_value = FindValue(arguments, "pids");
    const auto* pids = pids_value == nullptr
                           ? nullptr
                           : std::get_if<EncodableList>(pids_value);
    if (pids == nullptr || pids->empty()) {
      return EncodableValue(MakeFailure(
          "arguments", "No interference process targets were provided.",
          ERROR_INVALID_PARAMETER));
    }

    std::vector<std::string> args = {
        "manage-interference-processes",
        "--action",
        action,
    };
    for (const EncodableValue& pid_value : *pids) {
      int64_t pid = 0;
      if (const auto* typed = std::get_if<int64_t>(&pid_value)) {
        pid = *typed;
      } else if (const auto* typed32 = std::get_if<int32_t>(&pid_value)) {
        pid = *typed32;
      }
      if (pid > 0) {
        args.push_back("--pid");
    ...
    ParsedServiceResponse parsed;
    if (!RunServiceRequest(args, 10000, &parsed) || !parsed.ok) {
      const std::string error =
          parsed.error.empty()
              ? "TemnyiVPN service could not manage interference processes."
              : parsed.error;
      return EncodableValue(MakeFailure("service-interference", error));
    }

    EncodableMap response;
    response.insert_or_assign(EncodableValue("ok"), EncodableValue(true));
    response.insert_or_assign(
        EncodableValue("affectedPids"),
        EncodableValue(PidCsvToEncodableList(
            ServiceFieldValue(parsed.fields, "affectedPids"))));
    response.insert_or_assign(
        EncodableValue("failedPids"),
        EncodableValue(PidCsvToEncodableList(
            ServiceFieldValue(parsed.fields, "failedPids"))));
    response.insert_or_assign(
        EncodableValue("affectedCount"),
        EncodableValue(ParseServiceInt64(parsed.fields, "affectedCount", 0)));
    response.insert_or_assign(
        EncodableValue("failedCount"),
        EncodableValue(ParseServiceInt64(parsed.fields, "failedCount", 0)));
    response.insert_or_assign(
        EncodableValue("error"),
        EncodableValue(DecodeServiceField(parsed.fields, "errorB64")));
    response.insert_or_assign(
        EncodableValue("errorCode"),
        EncodableValue(ParseServiceInt64(parsed.fields, "errorCode", 0)));
    return EncodableValue(std::move(response));
  }
 ...
 private:
  ...
  EncodableMap StartInternal(const EncodableMap& arguments,
    ...
    if (requires_tun_prerequisites) {
      ...
      if (!elevated) {
        if (tun_ip_mode == "ipv4" && EnsureWindowsServiceReady()) {
          use_service = true;
          EmitLog("[app] TemnyiVPN Service helper is ready; privileged Windows TUN work is delegated.");
        } else {
          ...
          if (relaunch_map != nullptr && IsTruthy(*relaunch_map, "ok")) {
            ...
            response.insert_or_assign(
                EncodableValue("error"),
                EncodableValue(std::string(
                    "Elevated TemnyiVPN instance was launched.")));
            return response;
          }
    ...
    std::string tun_interface_name =
        requires_tun_prerequisites ? "TemnyiVPN TUN" : std::string();
  ...
  EncodableMap StartCoreViaService(const std::shared_ptr<RuntimeState>& state) {
    ...
    if (!RunServiceRequest(service_args, 10000, &parsed) || !parsed.ok) {
      return MakeFailure("service-core-start",
                         parsed.error.empty()
                             ? "TemnyiVPN Service failed to start core."
                             : parsed.error);
    }
    state->pid =
        static_cast<DWORD>(ParseServiceInt64(parsed.fields, "pid", 0));
    EmitLog("[app] Core process started by TemnyiVPN Service with PID " +
            std::to_string(state->pid) + ".");
  ...
  void AutoEngageKillswitchLocked(const std::string& core_binary_path) {
    ...
    if (!IsRunningAsAdministrator()) {
      EmitLog(
          "[app] Killswitch could not engage: TemnyiVPN service unavailable "
...
EncodableValue ManageWindowsInterferenceProcesses(
    const EncodableMap& arguments) {
  return WindowsRuntimeController::Instance().ManageInterferenceProcesses(
      arguments);
}
